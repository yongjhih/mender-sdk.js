...
export type PreAuthRequest = {
  /**
   * Precomputed device ID.
   */
  device_id: void | ?string,
  /**
   * Precomputed auth set ID.
   */
  auth_set_id: void | ?string,
  /**
...
All responses from the API will contain 'X-MEN-RequestID' header with server-side generated request ID.
 ...
 ==========================================================*/
   ...
   * @return Promise<any> -  -
   */
  postDevices(authSet: NewAuthSet): Promise<any> {
  ...
  getDeploymentsPaging(status: void | ?string, search: void | ?string, page: void | ?number, per_page: void | ?number, created_before: void | ?number, created_after: void | ?number, linked: boolean = true): Rx.Observable<Deployment> {
    ...
    .expand(res => {
      if (linked && res.headers && res.headers.link) {
        ...
        _page += 1;
        return Rx.Observable.fromPromise(get(status, search, _page, per_page, created_before, created_after));
      }
    })
    .map(res => res.data)
    .flatMap(it => Rx.Observable.from(it));
   ...
   * @return Promise<Device> -  -
   */
  getAuthDevices(page: ?number, per_page: ?number): Promise<Device> {
   ...
   * @return Promise<any> -  -
   */
  postAuthDevices(): Promise<any> {
    return this._axios.post(`/devauth/devices`).then(res => res.data);
  }
   ...
   * @return Promise<Device> -  -
   */
  getAuthDevicesById(id: string): Promise<Device> {
    return this._axios.get(`/devauth/devices/${id}`).then(res => res.data);
  }
   ...
   * @return Promise<any> -  -
   */
  deleteAuthDevicesById(id: string): Promise<any> {
    return this._axios.delete(`/devauth/devices/${id}`).then(res => res.data);
  }
   ...
   * @return Promise<any> -  -
   */
  deleteAuthDevicesByIdAuthByAid(id: string, aid: string): Promise<any> {
    return this._axios.delete(`/devauth/devices/${id}/auth/${aid}`).then(res => res.data);
  }
   ...
   * @return Promise<any> -  -
   */
  putAuthDevicesByIdAuthByAidStatus(id: string, aid: string): Promise<any> {
    return this._axios.put(`/devauth/devices/${id}/auth/${aid}/status`).then(res => res.data);
  }
   ...
   */
  getAuthDevicesCount(status: ?string): Promise<Count> {
   ...
   * @return Promise<any> -  -
   */
  deleteTokensById(id: string): Promise<any> {
    return this._axios.delete(`/devauth/tokens/${id}`).then(res => res.data);
  }
   ...
   */
  getLimitsMaxAuthDevices(): Promise<Limit> {
    return this._axios.get(`/devauth/limits/max_devices`).then(res => res.data);
  }
}
